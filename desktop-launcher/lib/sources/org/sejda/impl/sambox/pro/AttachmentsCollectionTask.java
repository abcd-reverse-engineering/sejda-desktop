...
public class AttachmentsCollectionTask extends BaseTask<AttachmentsCollectionParameters> {
    ...
    public void before(AttachmentsCollectionParameters parameters, TaskExecutionContext executionContext) throws TaskException {
        super.before((AttachmentsCollectionTask) parameters, executionContext);
    ...
    public void execute(AttachmentsCollectionParameters parameters) throws IllegalStateException, TaskException {
        int currentStep = 0;
        ...
        this.destinationDocument.getUnderlyingPDDocument().requireMinVersion(SpecVersionUtils.V1_7);
        ...
        collection.setItem(COSName.TYPE, (COSBase) COSName.getPDFName("Collection"));
        collection.setItem(COSName.getPDFName("Sort"), (COSBase) createSortDictionary());
        LOG.trace("Added sort dictionary");
        collection.setItem(COSName.getPDFName("Schema"), (COSBase) createSchemaDictionary());
        ...
        for (Source<?> source : parameters.getSourceList()) {
            ...
            map.put(currentStep + source.getName(), fileSpec);
        ...
        this.destinationDocument.catalog().getCOSObject().setItem(COSName.getPDFName("Collection"), (COSBase) collection);
