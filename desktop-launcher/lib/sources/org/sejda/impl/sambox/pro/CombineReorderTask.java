...
public class CombineReorderTask extends BaseTask<CombineReorderParameters> {
    ...
    public void before(CombineReorderParameters parameters, TaskExecutionContext executionContext) throws TaskException {
        super.before((CombineReorderTask) parameters, executionContext);
    ...
    public void execute(CombineReorderParameters parameters) throws IllegalStateException, TaskException {
        ...
        for (int j = 0; j < totalPages; j++) {
            FileIndexAndPage filePage = parameters.getPages().get(j);
            int fileIndex = filePage.getFileIndex();
            if (fileIndex >= 0 && this.documents.get(fileIndex) == null) {
                PdfSource<?> input = parameters.getPdfInputList().get(fileIndex).getSource();
