...
public class JpegToPdfTask extends BaseTask<JpegToPdfParameters> {
    ...
    private void executeOneOutput(JpegToPdfParameters parameters) throws TaskException {
        Rotation rotation;
        ...
        for (int i = 0; i < parameters.getSourceList().size(); i++) {
            Source<?> source = (Source) parameters.getSourceList().get(i);
            ...
            if (parameters.getRotations().size() > i && (rotation = (Rotation) parameters.getRotations().get(i)) != null && rotation != Rotation.DEGREES_0) {
                for (PDPage p : pages) {
        ...
        String outName = NameGenerator.nameGenerator(parameters.getOutputPrefix()).generate(NameGenerationRequest.nameRequest().originalName(((Source) parameters.getSourceList().get(0)).getName()));
