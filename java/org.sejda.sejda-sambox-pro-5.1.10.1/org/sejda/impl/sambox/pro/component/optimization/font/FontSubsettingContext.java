...
public class FontSubsettingContext {
    ...
    void maybeSetCurrentSubsettableFont(COSDictionary fontDictionary, COSName fontName, COSDictionary resources) {
        maybeSetCurrentSubsettableFont(fontDictionary, subFont -> {
            resources.setItem(fontName, subFont);
        });
    }
    ...
    private SubsettableInUseFont trueTypeSubsettableFont(COSDictionary fontDictionary, Consumer<SubsettableInUseFont> onSuccess) {
        try {
            COSStream ttfFontFile = (COSStream) Optional.ofNullable(fontDictionary.getDictionaryObject(COSName.FONT_DESC, COSDictionary.class)).map(d -> {
                return d.getDictionaryObject(COSName.FONT_FILE2, COSStream.class);
            }).orElse(null);
    ...
    private SubsettableInUseFont type0SubsettableFont(COSDictionary fontDictionary, Consumer<SubsettableInUseFont> onSuccess) {
        return (SubsettableInUseFont) Optional.ofNullable(fontDictionary.getDictionaryObject(COSName.DESCENDANT_FONTS, COSArray.class)).filter(a -> {
            return !a.isEmpty();
        }).map(a2 -> {
            return a2.getObject(0, COSDictionary.class);
        }).map(descendant -> {
            if (!isSubsetFontName(descendant.getNameAsString(COSName.BASE_FONT)) && COSName.CID_FONT_TYPE2.equals(descendant.getCOSName(COSName.SUBTYPE))) {
                COSStream ttfFontFile = (COSStream) Optional.ofNullable(descendant.getDictionaryObject(COSName.FONT_DESC, COSDictionary.class)).map(d -> {
                    return d.getDictionaryObject(COSName.FONT_FILE2, COSStream.class);
                }).orElse(null);
                if (Objects.nonNull(ttfFontFile)) {
    ...
    BaseTTFSubsetter subsetterFor(FontFileKey fontFileKey) {
        BaseTTFSubsetter baseTTFSubsetter = (Function) this.subsetters.get(fontFileKey);
        if (baseTTFSubsetter instanceof BaseTTFSubsetter) {
            BaseTTFSubsetter base = baseTTFSubsetter;
            return base;
        }
        return null;
    }
