...
public class ColorSpaceConverterOperator extends OperatorProcessorDecorator {
    ...
    public ColorSpaceConverterOperator(ContentStreamWriterStack writers, OperatorProcessor delegate) {
        ...
        setConsumer((operator, operands) -> {
            ...
            COSArray cOSArray = (COSBase) Optional.ofNullable(getContext().getResources()).map((v0) -> {
                return v0.getCOSObject();
            }).map(r -> {
                return r.getDictionaryObject(COSName.COLORSPACE, COSDictionary.class);
            }).map(d -> {
                return d.getDictionaryObject(name);
            }).orElse(null);
            if (COSName.PATTERN.equals(cOSArray)) {
                writeOperator(operands, operator);
                return;
            }
            if (cOSArray instanceof COSArray) {
                COSArray csArray = cOSArray;
                COSName csName = csArray.getObject(0);
